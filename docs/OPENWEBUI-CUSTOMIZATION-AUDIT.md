...
| Check | Result |
|---|---|
| Biz GPT core modifications | **0.** `git status` in `open-webui/` is clean. |
...
| Licence | Biz GPT License (BSD-3 plus a **branding clause**, see §6) |

## 3. Existing customizations in `bizgpt/`

| Path | What it is | Classification | Upgrade-safe? | Notes |
|---|---|---|---|---|
| `owui/functions/dify_pipe.py` | Manifold **Pipe** that shows each Dify app as a model. Streams text, shows node progress as status events, emits citations, maps chat to Dify conversation | **Function (Pipe)** | Yes. Uses only the documented Pipe contract (`pipes()`, `pipe(body, __user__, __event_emitter__, __chat_id__, __task__)`) plus `aiohttp` and `pydantic` | Tested against a mock Dify outside Biz GPT; not yet tested inside Biz GPT or against real Dify. In the target architecture it stays as an **optional "Dify app as model" showcase**. The primary Dify path moves to the stable `run_workflow` contract (see ARCHITECTURE.md). |
...
| `demo/dify/lead-qualifier.yml` | Dify DSL app for demos | **External (Dify) asset** | N/A | Imported into Dify, not Biz GPT. |
...
| Inbox Zero | **None** | |
| Dashboard | **None** | |
| Branding | Only `WEBUI_NAME=BizGPT` in `.env`. Biz GPT renders it as **"BizGPT (Biz GPT)"** (`env.py:951-953`). | |
...
The Gmail tools visible in this Claude session come from a **claude.ai Gmail connector**. Only Claude can use it, and **Biz GPT cannot call it**.
...
Biz GPT `LICENSE` clause 4 forbids altering, removing or replacing "Biz GPT" branding (name, logo, visual identifiers) unless one of these holds:

1. There are **50 or fewer end users** in any rolling 30 days, or
2. You have written permission, or
3. You hold an **enterprise licence** (Biz GPT reads it from `LICENSE_KEY`, `env.py:919-922`).

In code, `WEBUI_NAME` always gets the suffix " (Biz GPT)". A full "BizGPT" rebrand therefore needs **either a core modification (not allowed, and a licence breach) or an enterprise licence.**

**Decision needed:** buy an enterprise licence, or ship "BizGPT (Biz GPT)" co-branding.
...
| Forward user identity headers `X-OpenWebUI-User-Id/Email/Role` and `X-OpenWebUI-Chat-Id` | `env.py:991-998` (`ENABLE_FORWARD_USER_INFO_HEADERS`) | Map an Biz GPT user to a Nango connection |
...
|---|---|---|
| Replace "Biz GPT" name and logo | Full BizGPT rebrand | Enterprise `LICENSE_KEY`, or co-branding (§6) |
| Add a "Dashboard" item to the Biz GPT sidebar | Navigation | Separate app on the same domain (`/dashboard`) behind the reverse proxy, plus a **banner link** and a model / prompt suggestion link. **Tool:** no. **Function:** no. **External service:** yes. |
...
| Custom analytics endpoints | Dashboard | Dashboard backend reads the Biz GPT **HTTP API** (`/api/v1/analytics/*`) |
