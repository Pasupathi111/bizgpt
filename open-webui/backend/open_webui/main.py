...
async def chat_completion(
    ...
    try:
        ...
        # Provider models in app.state.MODELS do not always include workspace-model
        # metadata from the DB. Merge the custom model meta here so chat routing,
        # tool selection and capabilities respect the workspace model the user picked.
        model_context = copy.deepcopy(model)
        if model_info and model_info.meta:
            base_meta = ((model_context.get('info') or {}).get('meta') or {}).copy()
            custom_meta = model_info.meta.model_dump(exclude_none=True)
            base_capabilities = (base_meta.get('capabilities') or {}).copy()
            custom_capabilities = custom_meta.get('capabilities') or {}
            if custom_capabilities:
                custom_meta['capabilities'] = {**base_capabilities, **custom_capabilities}
            merged_meta = {**base_meta, **custom_meta}
            model_context['info'] = {**(model_context.get('info') or {}), 'meta': merged_meta}

        default_tool_ids = (((model_context.get('info') or {}).get('meta') or {}).get('toolIds') or None)
        if not form_data.get('tool_ids') and default_tool_ids:
            form_data['tool_ids'] = default_tool_ids

        # Read before the fallback below can rebind model to a different one.
        model_capabilities = ((model_context.get('info') or {}).get('meta') or {}).get('capabilities') or {}
        ...
        metadata = {
            ...
            or (((model_context.get('info') or {}).get('meta') or {}).get('toolIds') or None),
    ...
    if metadata.get('session_id') and metadata.get('chat_id'):
        ...
        metadata['message_id'] = message_ids[0]['message_id']
        return await process_chat(request, form_data, user, metadata, model_context, tasks)
